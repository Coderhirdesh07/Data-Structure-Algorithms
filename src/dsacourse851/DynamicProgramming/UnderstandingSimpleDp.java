...
//        dp[0][2] = arr[0][1];
    ...
    public static int question_26(int[] arr,int n){
        ...
        dp[1][0] = arr[0] + arr[1];
        dp[2][0] = Math.max(arr[0]+arr[1],arr[1]+arr[2]);
        dp[2][1] = arr[2]+arr[1] + arr[0];
        dp[3][0] = Math.max(arr[3]+arr[2],dp[2][0]);
        dp[3][1] = Math.max(arr[3]+arr[2]+arr[1],dp[2][1]);

        for(int i=4;i<n;i++){
            dp[i][0] = arr[i]+arr[i-1] + Math.max(dp[i-3][0],dp[i-4][1]);
            dp[i][1] = arr[i]+arr[i-1] + arr[i-2] + dp[i-4][0];
            if(i>=5){
                dp[i][1] = arr[i]+arr[i-1]+arr[i-2] + Math.max(dp[i-4][0],dp[i-5][1]);
            }
        }
        return Math.max(dp[n-1][0],dp[n-1][1]);
